...
coord = {
    ...
    "ho": 0,    # y-offset for individual article title
...
parser.add_argument('--font_face', nargs=2, default=['arial', 'arial_bold'],
...
# parse html ------------------------------------------------------------------
response = requests.get(journals[args.journal]['url'])
...
if args.bg == "none":
    img = Image.new("RGB", (1920, 1080), color = color_white)
else:
    img = Image.open(args.bg)

# parse and process highlights from page tree
highlights = [ process_highlight(x, highlight_img_width) for x in extract_highlights(soup, args.journal) ]
...
# save image
img.save(args.outdir + "/" + journals[args.journal]['filename'], quality = 100)
